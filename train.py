...
def main(args, init_distributed=False):
    ...
    if args.init_model_path is not None:
        checkpoint = torch.load(args.init_model_path, map_location='cpu')
        # model.load_state_dict(checkpoint['model'], strict=True)
        for name, w in model.named_parameters():
            if (name in checkpoint['model'].keys()) and ("decoder" in name) and ("out_proj.weight" in name):
                print(f'- Skip {name}')
            elif name in checkpoint['model'].keys():
                w.data = checkpoint['model'][name]
            else:
                print(f'- Skip {name}')
        del checkpoint
